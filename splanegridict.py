...
def init_argparse() -> argparse.ArgumentParser:
    ...
    parser.add_argument('-x', '--BCisString', dest='BCisString', action='store_true')
...
gridRangeCnt = ()
def readSpatial(infile, db, BCisString):
    global maxBarcodeLen
    global SpatialBarcodeRange_xXyY
    global GenesCnt
    global BarcodesCnt
    pbar = tqdm.tqdm(desc='Spatial', total=BarcodesCnt, ncols=70, mininterval=30, maxinterval=60, unit='', unit_scale=True, dynamic_ncols=True)
    with fileOpener(infile) as f:
        for index,line in enumerate(f, start=1):
            ...
            if BCisString:
                ...
                intSeq = dinopy.conversion.encode_twobit(seq.replace('N','A'))
...
def updateBarcodesID(infile, db, binPixels, BCisString):
    missingCnt = 0
    global gridRangeCnt
    global mgBoolMtx
    #eprint(len(mtxBar2sp))
    (gridRangeX, gridRangeY, gridCnt, minX, minY) = gridRangeCnt
    pbar = tqdm.tqdm(desc='Barcodes', total=BarcodesCnt, ncols=70, mininterval=300, maxinterval=500, unit='', unit_scale=True, dynamic_ncols=True)
...
def mkcopy(fromFile, toFile):
    fh = write2gzip(toFile)
    with bfileOpener(fromFile) as f:
        fh.write(f.read())
    fh.close()
...
def mkGridSpatial(spFile, scBarcodeFile, gridRangeCnt, binSize):
    ...
    for i in range(gridRangeCnt[2]):
        ...
        ### gridID = Xgrid * gridRangeY + Ygrid
        ### (gridRangeX, gridRangeY, gridCnt, minX, minY) = gridRangeCnt
        Xgrid = i // gridRangeCnt[1]
        Ygrid = i - (Xgrid * gridRangeCnt[1])
        Xpos = gridRangeCnt[3]+Xgrid*binSize
        Ypos = gridRangeCnt[4]+Ygrid*binSize
        barcodeStr = f"b{binSize}_{Xpos:05d}_{Ypos:05d}"
        #eprint(barcodeStr,str(Xgrid),str(Ygrid))
        print(barcodeStr, file=scFh)
        print(barcodeStr,Xpos,Ypos, file=spFh)
    spFh.close()
    scFh.close()
...
def main() -> None:
    ...
    eprint(f'[!]Treat Barcode as String:[{args.BCisString}], Debug Mode:[{args.dodebug}].')
    ...
    spStandardNameDict = dict(zip(spNameTuple,[ '.'.join((fn,'gz')) for fn in spFileNameList ]))
    InFileDict={}
    ...
    OutFileDict['rawbarcodes'] = args.outpath.joinpath('rawbarcodes.tsv.gz')
    OutFileDict['mgBoolMtx'] = args.outpath.joinpath('mgBoolMtx.mtx.gz')
    ...
    spatialDB = {}
    lineCnt = readSpatial(InFileDict['spatial'], spatialDB, args.BCisString)
    ...
    eprint("\tElapsed1 {}s".format((end1p - start)))
    ...
    missingCnt = updateBarcodesID(InFileDict['barcodes'], spatialDB, args.bin, args.BCisString)
    eprint('[!]Finished with [',missingCnt,'] missing barcodes.',sep='')
    if args.dodebug:
        mkcopy(InFileDict['barcodes'], OutFileDict['rawbarcodes'])
        fh = write2gzip(OutFileDict['mgBoolMtx'].as_posix())
        gb.io.mmwrite(target=fh, matrix=mgBoolMtx)
        fh.close()
    #spatialDB.close()
    end2p = time.perf_counter()
    eprint("\tElapsed2 {}s".format((end2p - end1p)))
    ...
    eprint("\tElapsed5 {}s".format((end5p - end4p)))

    executor.shutdown(wait=True)
    eprint('[!]All done !')
    eprint("\tTotal time {}s".format((end5p - start)))
    exit(0);
